...
export default function TenantMaintenance() {
  ...
  // ── Call maintenance-chat edge function ──────────────────────────────────
  async function callAI(messages: ChatMessage[]): Promise<string> {
    const { data, error } = await supabase.functions.invoke("maintenance-chat", {
      body: { title, messages },
    });
  ...
  async function startChat() {
    ...
    try {
      ...
      const aiMsg: ChatMessage = { role: "assistant", content: chat };
  ...
  async function sendMessage() {
    ...
    try {
      // Build messages to send (assistant first, alternating, matching Anthropic format)
      ...
      if (s) {
        await new Promise(r => setTimeout(r, 600)); // let final msg render
        setSummary(s);
              ...
              supabase.functions.invoke("send-transactional-email", {
                body: {
                  ...
                  templateData: {
                    ...
                    priority:    capturedPriority[0].toUpperCase() + capturedPriority.slice(1),
                  },
                },
              }),
                ...
                <>
                  <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Describe what's wrong and we'll guide you through the details.
                    </p>
                  </DialogHeader>
                  <div className="px-6 py-5 space-y-4">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Our assistant will ask a few quick questions so your landlord and any vendors
                      have everything they need to fix it fast.
                    </p>
                  </div>
                  ...
                  <DialogHeader className="px-6 pt-6 pb-4 border-b border-border">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Edit the summary if anything needs adjusting</p>
                      </div>
                    </div>
                  </DialogHeader>
